...
**The arancel block explains the gap instead of hiding it.** With no number at all it says so ("Consultá el arancel") rather than leaving the slot empty. **Amended by PR-33:** the 12-month rule no longer strips the amounts. A price past 12 months is rendered _with_ a warning banner above it naming the month it is from and telling the reader to confirm before deciding, and `verifiedAt` is what that banner is built from — the reason `PriceSummary` carries it on every price, fresh or not. Honest provenance beats a blank: it tells the student the number they may find elsewhere is old.
...
## 14. What PR-13 settled (the homepage)

**"Carreras más buscadas" shipped as "Carreras con más opciones", because we cannot measure the first one.** There is no per-career search volume anywhere in the system — `src/db/queries/events.ts` counts events by type, by day and by institution and by nothing else — and at launch the table is empty regardless. A popularity heading over a supply ranking is a claim about students we never observed, so the heading names the real quantity and the line under it says so out loud: _"Ordenadas por la cantidad de ofertas que publicamos en cada carrera, no por popularidad: no medimos qué busca la gente."_ Same rule as §12's accreditation ratio — the number is a fact about **what we published**, worded as such.

**The ranking is exact, not "the top áreas' top carreras".** No `topCareers` query exists and PR-13 was not the PR to add one, so `src/lib/home/top-careers.ts` walks áreas in descending order of published supply through `listCareersByArea` and stops as soon as the answer is settled: a carrera cannot have more ofertas than its own área, so once the eighth-best carrera held strictly beats the next área's total, no unvisited área can contain a contender. Strictly, not `>=` — an área whose whole supply ties the cut-off can still hold a carrera that ties it, and a tie is broken by name, not by which área we read first.

**The homepage has zero client components.** The brief expected the hero search to need client state; it does not. The shipped `SearchBar` is a GET form whose input is named from `FILTER_PARAMS`, so submitting it lands on a real `/carreras` URL in exactly the vocabulary `parseSearchFilters` reads back — the same "the honest HTML for changing filter state is navigation" decision §9 settled for the rail. Client state would have bought a suggestion dropdown nobody asked for and cost the first interaction on the most latency-sensitive page on the site. The route ships 0 B of route-level JS.

**There is no hero image, so the LCP element is the `h1`.** A hero photograph on a directory homepage is decoration that competes with the search field at 390px (§8.5 already asked for the field to be reachable without scrolling), and it would put the LCP on the network. What is above the fold at 390px is: what this is (`h1`), who it is for ("si estás eligiendo qué estudiar"), and what to do next (the field plus the accent CTA) — the three-question test, answered in server-rendered text.

**The accreditation numbers come from `listInstitutions()`, not from the `accreditationStatuses` facet.** A `vigente` facet count also contains CONES habilitaciones, because `vigente` is a status and the agency lives beside it — quoting that count as an ANEAES figure would be the exact conflation the accreditation wedge exists to correct (§11). `aneaesAccreditedCount` is agency-specific and is the same definition the institution profile already shows, so the homepage and the profile cannot disagree.

**The explainer teaser links to the filter, not to `/acreditacion`.** The hub is PR-30's and today the route is a placeholder that says "en construcción"; sending the homepage there is a dead link (§8.4). The teaser therefore carries the explanation itself — the three badges from §4 with one line each — and its action is `/carreras?acreditacion=vigente`, which works now and is built with `searchHref`, not a hand-written query string.

**The logo strip renders real logos or nothing: fewer than six and the section disappears.** `institutions.logo_url` is null for essentially every row until PR-19 ships uploads and settles R-08. Placeholder boxes, or the `InstitutionMonogram` standing in for a logo, would both dress an empty dataset as a roster of partners — the fabricated trust signal that must never appear here, alongside the student counts, ratings and testimonials this page also does not have. Six is the floor because five logos read as "the five we could get".

**That strip uses a plain `<img>`, one deliberate exception to `seo.md` §6.** `next/image` would have to run `unoptimized` — its loader validates the hostname against `images.remotePatterns`, and where logos will live is the R-08 decision PR-19 owns — and unoptimized it emits this same tag with the same `src` while adding ~10 kB of client runtime to the homepage for a section that currently renders nothing. Explicit dimensions and `loading="lazy"` are plain attributes. Revisit when PR-19 fixes the host.

**The one fade-up-on-scroll reveal §6 reserved for the homepage is not spent.** It needs an IntersectionObserver, which means the page's only client component and content that starts invisible — on the page whose whole job is to be legible in the first paint. The motion list is a ceiling, not a quota.
