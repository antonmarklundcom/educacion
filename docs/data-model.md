...
| `src/db/invariants.ts`       | PR-02    | `assertAccreditationCitation`, `assertAccreditationStatusIsSafe`, `assertScopeTarget`, `assertPriceIsCoherent`, `priceCheckViolations`, `computeAnnualCost`, `priceFreshness`. Every write path goes through these. (`isPriceDisplayable` was listed here until PR-48b; PR-33 deleted it and the table was not updated.) |
