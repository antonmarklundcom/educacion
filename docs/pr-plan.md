...
**Shipped as:** the five entities, but as **one generic route** (`/admin/[entity]`, Spanish keys) driven by a pure field registry (`src/lib/admin/entities.ts`) rather than five hand-written screen sets — fifteen screens would be fifteen places for a required-field rule or an enum vocabulary to drift, and the registry is what makes the parsing, the E.164/URL checks and the slug/`match_key` derivation unit-testable without a browser. `saveEntityAction` is the single write path for create _and_ edit, which is what makes "calls `requireRole`" and "logs before/after" properties of one function. Two decisions the plan did not specify are recorded in `architecture.md` §13: `enrollment_status` is `readOnly` in the registry and is skipped by the parser, so the cron keeps owning it and a forged field cannot become a column write; and the reindex is a 5 s debounce, because the rebuild replaces every row of `program_search` in a transaction and running it per save would hold that transaction over the table the public site is reading. **R-08 is closed with Cloudflare R2 over its S3-compatible API, signed in-process with `node:crypto`** (`architecture.md` §13.1, `deployment.md` §4) — configured as `S3_*` so Bunny is a value change, not a code change, and with a `UPLOADS_DIR` fallback for development. The "survives a simulated redeploy" criterion is tested as the property that makes it true: a deploy replaces the app directory, so both adapters _refuse at configuration time_ to root inside `process.cwd()` — `public/uploads` included. SVG is refused outright, which R-08 did not ask for and an origin we control requires.
