...
**Decided in PR-19: Cloudflare R2**, over its S3-compatible API, signed in-process with `node:crypto` — no SDK (`architecture.md` §13.1). The persistent-path alternative ships as the _development_ fallback (`UPLOADS_DIR`, default `~/educacion-uploads`, served through `/api/uploads/*`), not as the production answer: it survives a redeploy but not a slot migration, and it puts image bytes on the app server.

The risk is closed by a property rather than by a promise. Both adapters call `assertOutsideDeployDir()` and **throw at configuration time** if their root resolves inside `process.cwd()`, `public/uploads` included — so the failure mode this entry describes (it works locally, it disappears on the next deploy, somebody notices weeks later) cannot be configured. `src/lib/uploads/storage.test.ts` asserts it.

One thing R-08 did not mention and PR-19 had to decide: **SVG is refused.** An SVG is a document that can carry script and these bytes are served back from an origin we control, so an institution's own logo upload could otherwise run JavaScript on our domain. PNG, JPG and WebP, up to 256 kB.
