...
import { createAccreditationAction } from '../actions';
...
export default async function NewAccreditationPage({
  ...
  // The offering list is the only one that has to be scoped — programmes are a
  // few thousand and carry their institution's name, offerings are ~10 000.
  if (!institutionId) {
    return (
      <main className="mx-auto flex max-w-2xl flex-col gap-6 px-4 py-8 sm:px-6">
        <h1 className="text-ink text-2xl font-bold">Cargá una acreditación</h1>
        <p className="text-muted text-sm">
          Elegí primero la institución. Después vas a poder apuntar la acreditación a la institución
          entera, a un programa o a una oferta puntual.
        </p>
  ...
  return (
    <main className="mx-auto flex max-w-2xl flex-col gap-6 px-4 py-8 sm:px-6">
      ...
      <AdminForm
        ...
        defaultValues={{ institutionId, agency: 'ANEAES', kind: 'acreditacion' }}
        action={createAccreditationAction}
        submitLabel="Guardá la acreditación"
